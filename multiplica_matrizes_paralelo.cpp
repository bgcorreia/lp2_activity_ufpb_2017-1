...
void execThread(int i, int j, int cA, int cB, int *matrizA, int *matrizB, int *posMatrizC){
...
int main(){
	...
	for ( i = 0; i < lA; i++){
		for ( j = 0; j < cB; j++){
				/*  */
				objThread[index++] = thread(execThread, i, j, cA, cB, &matrizA[0][0], &matrizB[0][0], &matrizC[i][j]);
			}
	}
